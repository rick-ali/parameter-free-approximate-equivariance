...
Reads the .npz file and draws two scatter panels (PC1 vs PC2):
  ...
  right : rho_B  (digit-2 marginal, the ROTATED digit -> expect a spread orbit)
Each of the 8 points is labelled by its D4 element; points are connected in
...
def scatter_panel(ax, coords, var, codes, pred_unit, pcs, title):
    px, py = pcs
    xs, ys = coords[:, px], coords[:, py]
    # faint line through the points in D4-element order to show the orbit path
    ...
    for i in range(len(codes)):
        lab = f"{D4_LABELS[int(codes[i])]}"
        if pred_unit is not None:
            lab += f"→{int(pred_unit[i])}"  # predicted digit-2 class after this transform
...
def main():
    ...
    scatter_panel(axes[0], cA, vA, codes, pred_unit, (px, py),
                  "rho_A  (digit-1, fixed)")
    sc = scatter_panel(axes[1], cB, vB, codes, pred_unit, (px, py),
                       "rho_B  (digit-2, rotated)")
    cbar = fig.colorbar(sc, ax=axes, fraction=0.025, pad=0.02)
    cbar.set_label('D4 element (code)')

    sup = "Reduced-density PCA over digit-2's D4 orbit"

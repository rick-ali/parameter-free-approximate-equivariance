...
from utils.representations import C4xC4RegularRepresentation, D4xD4RegularRepresentation, OctahedralRegularRepresentation, D1RegularRepresentation

class MedMNISTGxGRegularFunctor(pl.LightningModule):
    def __init__(self, lr=0.001, gamma=0.1, milestones=None, output_root=None, data_flag=None,
        ...
        self._W_full_cache = {}
        self.task = INFO[data_flag]['task']

        assert '3d' in data_flag
        ...
        self.val_evaluator = Evaluator(data_flag, 'val', size=28)
        self.test_evaluator = Evaluator(data_flag, 'test', size=28)
        ...
        self.model = CNN3DResnet(n_classes=n_classes, n_channels=n_channels, mnist_type='single', get_latent=get_latent)
        ...
        if group == 'C4xC4':
            ...
            self.W = OctahedralRegularRepresentation(device=device)
        elif group == 'D1':
            self.W = D1RegularRepresentation(device=device)
        else:

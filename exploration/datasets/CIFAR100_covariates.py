...
class PairedCIFAR100Dataset(Dataset):
    def __init__(self, root="./dataset", train=True, transform=None, x2_angle=None):
        # Load the full MNIST dataset
        self.data = datasets.CIFAR100(root=root, train=train, download=True, transform=transform)
...
class PairedCIFAR100CovariatesDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        full_train_dataset = PairedCIFAR100Dataset(train=True, transform=self.transform, x2_angle=self.x2_angle)
        ...
        self.test_dataset = PairedCIFAR100Dataset(train=False, transform=self.transform, x2_angle=self.x2_angle)
...
if __name__ == "__main__":
    ...
    data_module = PairedCIFAR100CovariatesDataModule(batch_size=64, transform=transform, val_split=0.1)

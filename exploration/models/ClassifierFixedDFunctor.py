...
class ClassifierFixedDFunctor(pl.LightningModule):
    def __init__(self, latent_dim=32, learning_rate=0.003, lambda_t=0.01, lambda_c=0.01, D_values=None, num_classes=2):
        super(ClassifierFixedDFunctor, self).__init__()
        ...
        if D_values is None:
            self.D_values = torch.ones(latent_dim, device='cuda')
        else:
    ...
    def forward(self, x1, x2):
        ...
        D_proj = torch.diag(self.D_values)

        transformed = F.linear(encoded, D_proj)
    ...
    def on_test_epoch_end(self):
        D_values_dict = {}
        for x in self.D_values:
            x = x.item()
            D_values_dict[f'# {x} in D'] = torch.sum(self.D_values == x).item()

        self.log_dict(D_values_dict)

...
class RegularizedFunctor(pl.LightningModule):
    def __init__(self, 
                 latent_dim=32, 
                 learning_rate=0.003, 
                 lambda_t=0.01, 
                 lambda_W=0.001, 
                 init_W='identity', 
                 W_exponent_algebra=2,
                 run_id=None, 
                 save_W=True,
                 experiment_name='default'):
        super(RegularizedFunctor, self).__init__()
        ...
        if init_W == 'identity':
            self.W = torch.nn.Parameter(torch.eye(latent_dim, latent_dim))
        elif init_W == 'random':
            self.W = torch.nn.Parameter(torch.rand(latent_dim, latent_dim))
        elif init_W == 'fixed-1':
            self.W = -torch.eye(latent_dim, latent_dim, device='cuda')

        self.save_W = save_W
        self.suffix = self.lambda_W
        
        if self.save_W:
            self.save_dir = f'{experiment_name}/Winit={self.init_W}_latentdim={self.latent_dim}_lambdaW={self.lambda_W}'
    ...
    def loss_W_algebra(self):
        ...
        """
        modularity_penalty =  self.algebra_criterion(torch.linalg.matrix_power(self.W, self.W_exponent_algebra), torch.eye(self.latent_dim, device='cuda'))
        #modularity_penalty_2 = torch.dist(self.W @ self.W @ self.W, torch.linalg.pinv(self.W))
        #modularity_penalty_2 = self.algebra_criterion(self.W, torch.linalg.pinv(self.W))
        W_inverse = torch.linalg.solve(self.W, torch.eye(self.latent_dim, device='cuda'))
        modularity_penalty_2 = self.algebra_criterion(self.W, W_inverse)
    ...
    def shared_step(self, batch, batch_idx, stage):
        ...
        #loss_L = torch.relu(-self.encoder(x1)[:,0] * (1 - 2 * y1)).mean()
        loss_transformation = .95*loss_transformation_1 + .05*loss_transformation_2
        ...
        losses = {
                ...
                f'{stage}_loss_transformation_1': loss_transformation_1,
                f'{stage}_loss_transformation_2': loss_transformation_2,
                f'{stage}_loss_W_algebra': loss_W_algebra,
                f'{stage}_modularity_loss': modularity_penalty,
                #f'{stage}_loss_L': loss_L
            }
        
        loss = loss_reconstruction + self.lambda_t*loss_transformation + self.lambda_W*loss_W_algebra# + 0.001*loss_L
        if stage == 'train':
            losses['loss'] = loss
        else:
    ...
    def on_test_epoch_end(self):
        ...
        eig, _ = torch.linalg.eig(self.W)
        self.log_dict({'n_positive_eigenvalues' : eig[eig.real > 0].size().numel()})

...
class MLPNet(nn.Module):
    def __init__(
        self, in_channels, out_channels, h_size, w_size, hidden_dim, num_layers
    ):
        super(MLPNet, self).__init__()
        self.layers = [
            MLPBlock(in_channels * h_size * w_size, hidden_dim, h_size, w_size)
        ]
        self.layers += [
            MLPBlock(hidden_dim, hidden_dim, h_size, w_size)
    ...
    def forward(self, x):
        ...
        # exit()
        return self.model(x.view(x.shape[0], -1)).reshape(
            x.shape[0], -1, self.h_size, self.w_size
        )
...
class ConvNet(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_dim, kernel_size, num_layers, get_latents=False):
        super(ConvNet, self).__init__()
        print("THIS IS THE RIGHT CONVNET")
        self.get_latent = get_latents
...
class ENConv(torch.nn.Module):
    def __init__(self, in_frames, out_frames, kernel_size, N, L=2):
        super(ENConv, self).__init__()

        r2_act = gspaces.rot2dOnR2(N=N)
        feat_type_in = escnn.nn.FieldType(r2_act, in_frames * [r2_act.regular_repr])
        feat_type_hid = escnn.nn.FieldType(r2_act, out_frames * [r2_act.regular_repr])
        ...
        self.layer = escnn.nn.SequentialModule(
            ...
            escnn.nn.InnerBatchNorm(feat_type_hid),
...
class ENCNN(torch.nn.Module):
    def __init__(
        self, in_frames, out_frames, hidden_dim, kernel_size, num_layers, N, L=2
    ):
        super(ENCNN, self).__init__()
        r2_act = gspaces.rot2dOnR2(N=N)
        # r2_act = e2cnn.gspaces.Rot2dOnR2(N = N)

        self.feat_type_in = escnn.nn.FieldType(r2_act, in_frames * [r2_act.irrep(1)])
        # self.feat_type_in = e2cnn.nn.FieldType(r2_act, in_frames*[r2_act.irrep(1)])
        self.feat_type_hid = escnn.nn.FieldType(
            r2_act, hidden_dim * [r2_act.regular_repr]
        )
        # self.feat_type_hid = e2cnn.nn.FieldType(r2_act, hidden_dim*[r2_act.regular_repr])
        self.feat_type_out = escnn.nn.FieldType(r2_act, out_frames * [r2_act.irrep(1)])
        # self.feat_type_out = e2cnn.nn.FieldType(r2_act, out_frames*[r2_act.irrep(1)])
        input_layer = escnn.nn.SequentialModule(
            R2Conv(
                self.feat_type_in,
                self.feat_type_hid,
                kernel_size=kernel_size,
        ...
        layers = [input_layer]
        layers += [
            ENConv(hidden_dim, hidden_dim, kernel_size, N, L=L)
            for i in range(num_layers - 2)
        ]
        layers += [
            R2Conv(
    ...
    def forward(self, xx):
        xx = escnn.nn.GeometricTensor(xx, self.feat_type_in)
        out = self.model(xx)
        return out.tensor


class ENConvIrrep(torch.nn.Module):
    def __init__(
        self, feat_type_in, in_frames, out_frames, kernel_size, N, max_frequency=2
    ):
        super(ENConvIrrep, self).__init__()

        r2_act = gspaces.rot2dOnR2(N=N)
        feat_type_in = escnn.nn.FieldType(
            r2_act,
            in_frames * list(feat_type_in.representations),
        )
        feat_type_hid = escnn.nn.FieldType(
            r2_act,
            out_frames
            * escnn.group.directsum(
                [r2_act.irrep(i) for i in range(max_frequency + 1)]
            ),
        )
        self.fourier = escnn.nn.FourierPointwise(
            r2_act,
            out_frames,
            set(feat_type_hid.irreps),
            function="p_relu",
            N=12,
        )
        ...
        self.conv = R2Conv(
            feat_type_in,
            self.fourier.in_type,
            kernel_size=kernel_size,
            padding=(kernel_size - 1) // 2,
            basisexpansion="learn_eq_norm",
        )

        self.norm = escnn.nn.IIDBatchNorm2d(self.fourier.in_type)

        # self.layer = escnn.nn.SequentialModule(conv, norm, fourier)
...
class ENCNNIrrep(torch.nn.Module):
    def __init__(
        self,
        in_frames,
        out_frames,
        ...
        super(ENCNNIrrep, self).__init__()
        r2_act = gspaces.rot2dOnR2(N=N)
        ...
        # irreps = [r2_act.fibergroup.irrep(i) for i in range(max_frequency + 1)]

        irreps = r2_act.fibergroup.bl_irreps(L_feat)

        fourier = escnn.nn.FourierPointwise(
            r2_act,
            hidden_dim,
            irreps,
            function="p_relu",
        ...
        norm = escnn.nn.IIDBatchNorm2d(fourier.in_type)
        input_layer = escnn.nn.SequentialModule(conv, norm, fourier)

        layers = [input_layer]
        for i in range(num_layers - 2):
            fourier_sub = escnn.nn.FourierPointwise(
                r2_act,
                hidden_dim,
                irreps,
                function="p_relu",
            ...
            norm_sub = escnn.nn.IIDBatchNorm2d(fourier_sub.in_type)
            layers.append(escnn.nn.SequentialModule(conv_sub, norm_sub, fourier_sub))

        # layers += [
        #     ENConvIrrep(fourier.out_type, hidden_dim, hidden_dim, kernel_size, N)
        #     for i in range(num_layers - 2)
        # ]
...
class Lift_Rot_Expansion(torch.nn.Module):
    def __init__(
        ...
        self.feat_type_in = e2cnn.nn.FieldType(r2_act, in_frames * [r2_act.irrep(1)])
        self.feat_type_hid = e2cnn.nn.FieldType(
            r2_act, backbone_hidden_dim * [r2_act.regular_repr]
        )
        self.feat_type_hid_2 = e2cnn.nn.FieldType(
            r2_act,
            (backbone_hidden_dim + backbone_hidden_dim // 4) * [r2_act.regular_repr],
        )
        self.feat_type_out = e2cnn.nn.FieldType(r2_act, out_frames * [r2_act.irrep(1)])
...
class ConvE2CNN(torch.nn.Module):
    def __init__(
        ...
        self.convnet += [
            ConvBlock(conv_hidden_dim, conv_hidden_dim, kernel_size)
            for i in range(conv_num_layers - 2)
        ]
        ...
        r2_act = e2cnn.gspaces.Rot2dOnR2(N=N)
        self.feat_type_hid = e2cnn.nn.FieldType(
            r2_act, e2cnn_hidden_dim * [r2_act.regular_repr]
        )
        self.feat_type_out = e2cnn.nn.FieldType(r2_act, out_frames * [r2_act.irrep(1)])

        self.e2cnn = [
            E2Conv(e2cnn_hidden_dim, e2cnn_hidden_dim, kernel_size, N)
            for i in range(e2cnn_num_layers - 2)
        ]
        self.e2cnn += [
            e2cnn.nn.R2Conv(
                self.feat_type_hid,
...
class RPPBlock_Irrep(nn.Module):
    def __init__(
        self,
        in_frames,
        out_frames,
        ...
        super(RPPBlock_Irrep, self).__init__()
        r2_act = escnn.gspaces.rot2dOnR2(N=N)
        self.first_layer = first_layer
        self.final_layer = final_layer
        ...
        if self.first_layer:
            ...
            self.feat_type_in = escnn.nn.FieldType(
                r2_act, in_frames * [r2_act.irrep(i) for i in range(2 + 1)]
            )
        ...
        if self.final_layer:
            ...
            self.activation = escnn.nn.FourierPointwise(
                r2_act,
                function="p_relu",
                irreps=r2_act.fibergroup.bl_irreps(2),
                channels=out_frames,
                N=12,
            )
            self.feat_type_hid = self.activation.in_type
            self.norm = escnn.nn.IIDBatchNorm2d(self.activation.out_type)

        self.e2cnn = escnn.nn.R2Conv(
            self.feat_type_in,
            self.feat_type_hid,
            kernel_size=kernel_size,
            padding=(kernel_size - 1) // 2,
    ...
    def forward(self, x):
        convout = self.conv(x)
        e2cnnout = self.e2cnn(escnn.nn.GeometricTensor(x, self.feat_type_in)).tensor
        if self.final_layer:
            return convout + e2cnnout
        else:
            out = escnn.nn.GeometricTensor(convout + e2cnnout, self.e2cnn.out_type)
            return self.activation(self.norm(out))
...
class Rot_RPPNet(nn.Module):
    def __init__(
        ...
        rpp = RPPBlock_Irrep if N == -1 else RPPBlock
        self.layers = [
            rpp(
                in_frames=in_frames,
...
class Constrained_LCBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, h_size, w_size, final_layer=False
    ):
        super(Constrained_LCBlock, self).__init__()
        torch.manual_seed(0)
        self.weights = nn.Parameter(
            torch.randn(
                h_size, w_size, out_channels, in_channels, kernel_size, kernel_size
            )
            .float()
            .to(device)
            / in_channels
        ...
        self.bias = nn.Parameter(
            torch.randn(out_channels).float().to(device) / in_channels
        )
    ...
    def get_rotated_kernels(self, theta):
        ...
        temp_w = temp_w.reshape(-1, 2, self.kernel_size, self.kernel_size)
        temp_w = torch.cat(
            [self.rot_img(temp_w[:, :1], theta), self.rot_img(temp_w[:, 1:2], theta)],
            dim=1,
        )
        temp_w = temp_w.reshape(
            self.h_size,
            self.w_size,
            self.out_channels // 2,
            2,
        ...
        temp_w = temp_w.reshape(
            -1, 2, self.in_channels, self.kernel_size, self.kernel_size
        )
        temp_w = self.rot_vector(temp_w, theta)
        temp_w = temp_w.reshape(
            self.h_size,
    ...
    def forward(self, x):
        x = F.unfold(
            x, kernel_size=self.kernel_size, padding=(self.kernel_size - 1) // 2
        )
        x = x.reshape(
            x.shape[0], self.in_channels, self.kernel_size, self.kernel_size, -1
        )
        x = x.reshape(
            x.shape[0],
            self.in_channels,
...
class Constrained_Rot_LCNet(nn.Module):
    def __init__(
        ...
        layers = [
            Constrained_LCBlock(in_channels, hidden_dim, kernel_size, h_size, w_size)
        ]
        layers += [
            Constrained_LCBlock(hidden_dim, hidden_dim, kernel_size, h_size, w_size)
            for i in range(num_layers - 2)
        ]
        layers += [
            Constrained_LCBlock(
                hidden_dim, out_channels, kernel_size, h_size, w_size, final_layer=True
            )
        ]
    ...
    def get_weight_constraint(self):
        ...
        for layer in self.rconv:
            for j in range(1, self.N):
                constraint += F.mse_loss(
                    layer.weights, layer.get_rotated_kernels(theta_step * j)
                )
...
class Relaxed_Rot_SteerConv(torch.nn.Module):
    def __init__(
        ...
        self.bias = nn.Parameter(
            torch.zeros(out_frames * self.basis_kernels.shape[1]).to(device)
        )
        self.bias.data.uniform_(-stdv, stdv)
    ...
    def forward(self, x):
        conv_filters = torch.einsum(
            "bpqk,obik->opiqk",
            self.basis_kernels.to(self.relaxed_weights.device),
        ...
        # print("heyoo")
        # print(self.basis_kernels.shape)
        # print(self.relaxed_weights.shape)
        # print(conv_filters.shape)

        if not self.last_layer:
            out = F.conv2d(x, conv_filters, self.bias, padding=1)
            return self.relu(
                e2cnn.nn.GeometricTensor(out, self.feat_type_hid)
            ).tensor  # self.norm(
        else:
...
class Relaxed_Rot_SteerConvIrrep(torch.nn.Module):
    def __init__(
        self,
        in_frames,
        out_frames,
        ...
        super(Relaxed_Rot_SteerConvIrrep, self).__init__()
        r2_act = escnn.gspaces.rot2dOnR2(N=N)
        self.last_layer = last_layer
        self.first_layer = first_layer
        self.kernel_size = kernel_size
        ...
        if self.first_layer:
            ...
            self.feat_type_in = escnn.nn.FieldType(
                r2_act,
                in_frames
                * [escnn.group.directsum([r2_act.irrep(i) for i in range(L_feat + 1)])],
            )
        ...
        if not last_layer:
            self.relu = escnn.nn.FourierPointwise(
                r2_act,
                irreps=set(self.feat_type_hid.irreps),
                channels=out_frames,
                function="p_relu",
                N=12,
                type="regular",
            )
            self.norm = escnn.nn.IIDBatchNorm2d(self.feat_type_hid)

        # grid, basis_filter, rings, sigma, maximum_frequency = compute_basis_params(
        #     kernel_size=kernel_size
        # )

        grid = escnn.nn.modules.conv.rd_convolution.get_grid_coords(2, kernel_size)

        basis_filter, rings, sigma, maximum_frequency = (
            escnn.nn.modules.conv.r2convolution.compute_basis_params(kernel_size)
        )
        ...
        block_expansion = escnn.nn.modules.basismanager.basisexpansion_singleblock.block_basisexpansion(
            basis, grid, basis_filter, recompute=False
        )
        # grid = torch.tensor(grid)
        # print(len(torch.unique(torch.atan2(grid[:, 0], grid[:, 1]))))
    ...
    def forward(self, x):
        ...
        if not self.last_layer:
            out = F.conv2d(
                x, conv_filters, self.bias, padding=(self.kernel_size - 1) // 2
            )
            return self.relu(
                escnn.nn.GeometricTensor(out, self.relu.in_type)
            ).tensor  # self.norm( TODO check norm
        else:
            return F.conv2d(
                x, conv_filters, self.bias, padding=(self.kernel_size - 1) // 2
            )
...
class Relaxed_Rot_SteerConvNetIrrep(torch.nn.Module):
    def __init__(
        self,
        in_frames,
        out_frames,
        ...
        super(Relaxed_Rot_SteerConvNetIrrep, self).__init__()
        self.alpha = alpha

        layers = [
            Relaxed_Rot_SteerConvIrrep(
                in_frames=in_frames,
                out_frames=hidden_dim,
                kernel_size=kernel_size,
                N=N,
...
# thing = Relaxed_Rot_SteerConvNetIrrep(10, 6, 96, 5, 5, -1).to(device)
# thing = ENCNNIrrep(10, 6, 96, 5, 5, -1, L=2).to(device)

# array = torch.randn(32, 2, 32, 32).to(device)

# thing(array)

# print(sum(p.numel() for p in thing.parameters() if p.requires_grad) / 1e6)
...
class Relaxed_TR_SteerConv(nn.Module):
    def __init__(
        ...
        self.combination_weights = nn.Parameter(
            torch.ones(h_size, w_size, num_banks).float().to(device) / num_banks
        )
    ...
    def forward(self, x):
        ...
        if self.last_layer:
            ...
            return (
                self.convs[0]
                .relu(e2cnn.nn.GeometricTensor(out, self.convs[0].feat_type_hid))
                .tensor
            )
